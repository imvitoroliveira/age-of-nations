...
export const FarmGame = ({ onBack }: Props) => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      {/* Sky */}
      <div className="absolute inset-0 transition-all duration-[30000ms]"
        style={{ background: getSkyGradient() }} />

      {/* Clouds */}
      <div className="absolute top-[5%] left-0 w-full h-16 pointer-events-none overflow-hidden z-[1]">
        {[0, 1, 2].map(i => (
          <motion.span key={i} className="absolute text-lg opacity-50"
            style={{ top: `${i * 30}%` }}
            animate={{ x: ['-10%', '110%'] }}
      ...
      {/* Sun / Moon */}
      <div className="absolute top-3 z-[2] pointer-events-none transition-all duration-1000"
        style={{ left: `${celestialX}%`, transform: `translateY(${celestialY}px)` }}>
        <span className="text-2xl drop-shadow-lg" style={{ filter: isNight ? 'none' : 'drop-shadow(0 0 8px #FFD700)' }}>
          {isNight ? '🌙' : '☀️'}
        </span>
      </div>
        ...
        <div className="absolute inset-0 bg-[rgba(0,0,20,0.45)] pointer-events-none z-[3] transition-opacity duration-[3000ms]">
          {Array.from({ length: 10 }).map((_, i) => (
            <motion.div key={i} className="absolute w-0.5 h-0.5 bg-white rounded-full"
              style={{ left: `${10 + i * 9}%`, top: `${5 + (i * 7) % 25}%` }}
      ...
      {weather === 'rainy' && (
        <div className="absolute inset-0 pointer-events-none z-[3] overflow-hidden">
          {Array.from({ length: 15 }).map((_, i) => (
            <motion.div key={i} className="absolute w-px bg-blue-300/30"
              style={{ left: `${i * 7}%`, height: '15px' }}
              animate={{ y: ['-10%', '110%'] }}
      ...
      <div className="absolute z-[1] pointer-events-none" style={{ bottom: '55%', left: 0, right: 0 }}>
        <svg viewBox="0 0 1200 120" className="w-full" preserveAspectRatio="none" style={{ height: '40px' }}>
      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-2 md:p-4">
          <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ border: '3px solid #7a5c2e', maxWidth: 'min(90vw, 700px)' }}>
            <FarmGrid />
            <Pasture />
          </div>
        </div>
        ...
        {selectedCrop && (
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 px-3 py-1.5 rounded-xl"
            style={{ background: 'rgba(20, 10, 5, 0.9)', border: '2px solid #7a5c2e', fontFamily: "'Press Start 2P', monospace", fontSize: '8px', color: '#FFD700' }}>
