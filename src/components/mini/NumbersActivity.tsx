...
import { NUMBERS, NUMBER_EMOJIS } from '@/data/educationData';
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 p-4">
          <div className="flex justify-center mb-4">
            <button onClick={() => setMode('count')} className="kid-btn bg-secondary text-secondary-foreground">
              Contar! 🧮
            </button>
          </div>
          <div className="grid grid-cols-5 gap-4 max-w-md mx-auto">
            {NUMBERS.map(n => (
              <button
                key={n}
                onClick={() => speak(`${n}`)}
                className="kid-card bg-card aspect-square flex flex-col items-center justify-center border-secondary/30"
              >
                <span className="text-4xl font-extrabold font-baloo text-secondary">{n}</span>
                <span className="text-xs mt-1">{Array(n).fill('⭐').join('')}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 gap-6">
        ...
        <div className="flex flex-wrap justify-center gap-3 max-w-xs">
          {Array(targetNum).fill(0).map((_, i) => (
            <span key={i} className="text-5xl bounce-in" style={{ animationDelay: `${i * 0.1}s` }}>
              {countEmoji}
            </span>
          ))}
        </div>
