...
export const MathActivity = ({ onBack }: Props) => {
  ...
  const handleAnswer = (val: number) => {
    ...
    if (correct) {
      ...
      setStreak(newStreak);
      if (newStreak >= 5 && difficulty === 'easy') setDifficulty('medium');
      else if (newStreak >= 10 && difficulty === 'medium') setDifficulty('hard');
    } else {
      setStreak(0);
    }
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 gap-8">
        <div className="flex gap-2 items-center">
          {['easy', 'medium', 'hard'].map(d => (
            <span key={d} className={`px-3 py-1 rounded-full text-sm font-bold ${
              d === difficulty ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
            }`}>
              {d === 'easy' ? '⭐' : d === 'medium' ? '⭐⭐' : '⭐⭐⭐'}
            </span>
          ))}
          <span className="ml-2 text-sm text-muted-foreground">Sequência: {streak} 🔥</span>
        </div>
