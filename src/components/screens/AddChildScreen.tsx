          ...
          <div className="flex flex-wrap gap-2 justify-center mt-3">
            {AVATAR_EMOJIS.map(e => (
              <button key={e} onClick={() => setAvatar(e)}
                className={`text-3xl p-2 rounded-xl transition-all ${e === avatar ? 'bg-primary/20 scale-110 ring-2 ring-primary' : 'hover:bg-muted'}`}>{e}</button>
            ))}
          </div>
        </div>
        <div className="w-full">
          <label className="text-lg font-bold mb-2 block">Nome</label>
          <Input value={name} onChange={e => setName(e.target.value)} placeholder="Nome da criança" className="text-xl h-14 rounded-2xl text-center" maxLength={20} />
        </div>
        <div className="w-full">
          ...
          <div className="flex gap-2 justify-center">
            {[1, 2, 3, 4, 5, 6].map(a => (
              <button key={a} onClick={() => setAge(a)}
                className={`w-14 h-14 rounded-2xl text-2xl font-bold transition-all ${a === age ? 'bg-primary text-primary-foreground scale-110' : 'bg-muted text-muted-foreground'}`}>{a}</button>
            ))}
          </div>
